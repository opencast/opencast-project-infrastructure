...
def getPullRequestSchedulers():
  return schedulers.AnyBranchScheduler(name="Pull Requests",
...
def getSchedulers(pretty_branch_name, git_branch_name):
  ...
  commits_branch = schedulers.AnyBranchScheduler(name=pretty_branch_name + " Commits",
                                                  change_filter=util.ChangeFilter(category=None, branch_re=git_branch_name),
                                                  treeStableTimer={{ stability_limit }}, #NB: Do not make this a string, a horribly unclear error occurs and nothing works for this scheduler...
                                                  builderNames=[pretty_branch_name + " Build", pretty_branch_name + " Reports", pretty_branch_name + " Markdown docs"])

  nightly_branch = schedulers.Nightly(name=pretty_branch_name + ' Nightly',
                                     change_filter=util.ChangeFilter(category=None, branch_re=git_branch_name),
                                     hour=3, onlyIfChanged=True,
                                     builderNames=[pretty_branch_name + " Nightly", pretty_branch_name + " Reports", pretty_branch_name + " Markdown docs"])

  forceScheduler = schedulers.ForceScheduler(name="ForceBuildCommits" + git_branch_name,
